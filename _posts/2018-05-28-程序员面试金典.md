...
public class Same {
    public boolean checkSam(String stringA, String stringB) {
        ...
        Map<Character, Integer> stringBMap = new HashMap<>();
        for(int i=0; i<stringA.length(); i++) {
            stringAMap.put(stringA.charAt(i), stringAMap.getOrDefault(stringA.charAt(i), 0)+1);
        }
        for(int i=0; i<stringB.length(); i++) {
            stringBMap.put(stringB.charAt(i), stringBMap.getOrDefault(stringB.charAt(i), 0)+1);
        }
        for(Map.Entry<Character, Integer> entry:stringAMap.entrySet()) {
            if(entry.getValue()!=stringBMap.getOrDefault(entry.getKey(), 0))
                return false;
        }
        return true;
    }
