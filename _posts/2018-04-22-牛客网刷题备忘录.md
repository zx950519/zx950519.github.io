...
public class Solution {
    public int FirstNotRepeatingChar(String str) {
        ...
        for (int i=0; i<tmp.length; i++) {
            data.put(tmp[i], data.getOrDefault(tmp[i], 0)+1);
        }
        for (int i=0; i<tmp.length; i++) {
            if (data.getOrDefault(tmp[i], 0) == 1) 
                return i;
        }
        return -1;
    }
...
public class Solution {
    public boolean IsPopOrder(int [] pushA,int [] popA) {
        ...
        for(int i=0; i<pushA.length; i++) {
            sta.push(pushA[i]);
            while(!sta.empty() && sta.peek() == popA[index]) {
                sta.pop();
                index++;
            }
        }
...
```
![](http://ww1.sinaimg.cn/large/005L0VzSgy1fr6k5c27ipj30d906s0sx.jpg)
```
