...
title: CentOS7生产环境搭建
catalog: Shell
description: 各种开发环境在CentOS7下的部署
keywords: Shell
---
...
&emsp;&emsp;使用IDEA写代码，打包后使用/bin/spark-submit提交，其他参数后续将会解释  

&emsp;&emsp;在IDEA中用sbt构建项目  
...
```
&emsp;&emsp;然后执行打包操作,具体如下:File -> Project Structure -> Aritifacts -> 点击+号 ->jar -> 第二个 -> 指定Module和MainClass -> JAR files from libraries 选择第二个 ->点击ok;主题栏点击Build -> Build Aritifacts - Build;在工程目下out目录中生成相应jar包即打包成功,在下图中，当我们看到sr.jar即说明搞定，我们这个项目的名字就叫sr(shit，我想抽ssr啊)  

![](http://ww1.sinaimg.cn/large/005L0VzSly1ftky6ijqz9j307f092q31.jpg)  

&emsp;&emsp;先上传相关代码所需输入文件，贴几个常见的hdfs命令  
...
&emsp;&emsp;在spark的目录下执行:"./bin/spark-submit --master spark://master:7077 --class WordCount /home/hadoop/sr.jar"  
&emsp;&emsp;说明:参数master指定谁是控制机器;参数Class指定主类以及之前打包的项目位置  
&emsp;&emsp;可以在浏览器中查看运行结果(master:8080)  
...
&emsp;&emsp;1.下载压缩包  
&emsp;&emsp;2.解压(tar -zxvf apache........)  
&emsp;&emsp;3.配置环境变量(su root; vim /etc/profile)，如下：  
```
...
```
&emsp;&emsp;4.刷新(source /etc/profile)  
&emsp;&emsp;5.验证(mvn -v)

## protobuf安装

&emsp;&emsp;1.下载压缩包(wget https://github.com/google/protobuf/releases/download/v2.5.0/protobuf-2.5.0.tar.gz)  
&emsp;&emsp;2.解压缩并进入文件夹(tar -zxvf protobuf-2.5.0.tar.gz)  
&emsp;&emsp;3.执行(./configure)  
&emsp;&emsp;4.执行(make && make install)  
&emsp;&emsp;5.修改环境变量(export PATH=/home/hadoop/protobuf-2.5.0/bin:$PATH)  
&emsp;&emsp;6.验证(protoc --version), 出现libprotoc 2.5.0 则说明安装成功
...
- 修改配置文件:vim /etc/exports;新增一行:/home/hadoop/share 10.1.18.0/24(rw,async,insecure,anonuid=0,anongid=0,no_root_squash)或者(rw,anonuid=0,anongid=0,all_squash,sync)  
...
```
- 使配置生效:exportfs -rv  
- 启动并设置开机自动启动:systemctl enable rpcbind;systemctl start rpcbind;systemctl enable nfs-server;systemctl start nfs-server  
- 查看启动情况:rpcinfo -p
- 对共享文件夹授权:chomd 777 -R /home/hadoop/share

#### 客户端配置

- 安装命令:yum install -y nfs-utils rpcbind
- 设置开机自动启动:chkconfig nfs on; chkconfig rpcbind on
- 启动服务:service rpcbind start; service nfs start
- 创建挂载点:mount -t nfs server_ip:/home/hadoop/share /home/hadoop/share //注释-服务器地址:/服务器下共享文件夹的路径 本地的共享文件夹路径
- 查看挂载目录:df -h
- 卸载挂载目录:umount /home/hadoop/share
- 开机自动挂载:sudo vim /etc/fstab; 添加一行:server_ip:/home/hadoop/share /home/hadoop/share nfs rw,tcp,intr 0 1

#### 故障处理

&emsp;&emsp;重启服务器端：service nfs reload  
&emsp;&emsp;重启服务器端：service nfs restart  

## NTP时间校准

#### 服务器端设置

- 安装/更新ntp:yum install -y ntp
- 先校准时间:ntpdate cn.pool.ntp.org
- 更改配置:sudo vim /etc/ntp.conf
```
...
```
- 启动/重启ntp:service ntpd start(restart)
- 查看ntp情况:ntpq -p
