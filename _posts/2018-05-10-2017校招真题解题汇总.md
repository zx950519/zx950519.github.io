...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        if(data.length()<=6) {
            ...
            vector.add(data.substring(0,6));
            vector.add(data.substring(6));
        }
        else {
            vector.add(data.substring(0, 6));
            vector.add(data.substring(6, 14));
            vector.add(data.substring(14));
        }
        for(int i=0; i<vector.size(); i++) {
            if(i > 0) {
                System.out.print(" ");
...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        Map<Integer, Integer> mmp = new HashMap<>();
        for(int i=0; i<data.length; i++) {
            mmp.put(Integer.parseInt(data[i]), 1);
        }
        for(int i=1; i<=10000; i++) {
            if(mmp.getOrDefault(i, 0)==0)
                vector.add(i);
        }
        ...
        Long td = Long.parseLong(dp);
        System.out.print(td%7);
    }
}
```
...
public class Main{
    public static void main(String[] args) throws Exception{
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(System.in));
        String[] data = bufferedReader.readLine().split(" ");
        int size = Integer.parseInt(data[data.length-1]);
        Vector<Integer> vector = new Vector<>();
        for(int i=0; i<data.length-1; i++)
            vector.add(Integer.parseInt(data[i]));
        Collections.sort(vector);
...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        for(int i=1; i<tmp.length; i++) {
            sol[i] = Math.max(sol[i-1], 0) + tmp[i];
            if(sol[i] > max)
                max = sol[i];
        }
...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        while(size > 0) {
            int num = Integer.parseInt(bufferedReader.readLine());
            LinkedList<Integer> data = Solution(num);
    ...
    public static LinkedList<Integer> Solution(int n){
...
public class Main{
    public static void main(String[] args) throws Exception{
        ...
        Long m = Long.parseLong(data[1])-1;
...
//        System.out.println("正在处理："+index+" 目前总和为："+sum);
        if(sum>m) {
            return;
        }
        if(sum==m){
//            System.out.println("已经找到一种解决方案了");
            for(int i=0; i<vector.size(); i++) {
                if(i!=0)
                    System.out.print(" ");
                System.out.print(vector.get(i));
...
//            System.out.println("正在处理i="+i+"的情况");
            vector.add(i);
            dfs(i+1, sum+i);
